#!/usr/bin/env python3
"""
Stage 5: monocular / multi-view depth with Depth Anything 3 (DA3-BASE).

Reads RGB images from ``output/images``, runs ``DepthAnything3`` from the Hugging Face
weights ``depth-anything/da3-base``, and writes one depth file per source image under
``output/depth`` using the **same base name** as the input (``.png``, 16-bit relative depth).

Requires the Depth Anything 3 package (not on PyPI as a simple pip name):
